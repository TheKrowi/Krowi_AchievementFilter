...
local iterations, insertions, titlesFound;
local function FindAchievement(tbl, depth, maxDepth)
    ...
    for key, value in pairs(tbl) do
        if type(value) == "table" then
            FindAchievement(value, depth + 1, maxDepth);
        end
        if key == "achID" or key == "achievementID" then
            isAchievement = true;
        elseif key == "titleID" and tbl.parent.g[1].parent.achID then
            if not KrowiAF_ATT[tbl.parent.g[1].parent.achID] then
                insertions = insertions + 1;
                KrowiAF_ATT[tbl.parent.g[1].parent.achID] = {};
            end
            KrowiAF_ATT[tbl.parent.g[1].parent.achID].title = value;
            titlesFound = titlesFound + 1;
        end
    end

    if isAchievement then
        if not KrowiAF_ATT[tbl.achID] then
            insertions = insertions + 1;
            KrowiAF_ATT[tbl.achID] = {};
        end
        for key, value in pairs(tbl) do
            if type(value) ~= "table" and key ~= "achID" and key ~= "achievementID" and key ~= "sourceIgnored" and key ~= "description" then
                KrowiAF_ATT[tbl.achID][key] = value;
            end
        end
    end
end

function KrowiAF_GetATTAchInfo(depth, maxDepth)
    KrowiAF_ATT = {};

    iterations, insertions, titlesFound = 0, 0, 0;
    FindAchievement(KrowiATT.Categories.Achievements, depth, maxDepth)
    print(iterations, insertions, titlesFound)
end

function KrowiAF_CleanATTAchInfo()
    for key, value in pairs(KrowiAF_ATT) do
        local items = 0;
