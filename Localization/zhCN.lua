...
-- [[ Exported at 2026-06-17 18-16-17 ]] --
L["%c"] = true
L["%d/%m/%Y %I:%M %p"] = true
L["%m/%d/%Y %I:%M %p"] = true
L["%m/%d/%Y %R"] = true
...
|T:1:8|t- 更高亮显示鼠标点击的日期（18日）并保持右侧窗口（锁定），可上下滚动查看成就、悬停鼠标查看提示或点击跳转到成就窗口（将关闭日历窗口），仅可选择获得过成就的日期
|T:1:8|t- 再次点击所选日期将取消选择并解锁右侧窗口，也可点击右侧窗口的关闭按钮取消选择
|T:1:8|t- 日期上的箭头标志表示当日获得过4个以上成就

设置(1)
|T:1:8|t- 可选择在打开日历时总是保留关闭前的月份（{lockMonth}）或因点击成就而关闭时保留（{lockAchievementMonth}）
|T:1:8|t- 可更改每周的第一天，当前设置为{firstWeekDay}
...
|T:1:8|t- {watchListCategory}: 显示已关注成就，可设置(1)是否显示子类
|T:1:8|t- {excludedCategory}: 显示已关注成就，可设置(1)是否显示子类，可在设置(1)中永久隐藏
...
L["Full Search"] = "完整搜索"
L["Full Search Desc"] = "选择“{showAllResults}”可查看所有搜索结果（查看{quickSearchTutorial}教程了解更多细节）"
...
|T:1:8|t- 可快速{goTo}所在的{partOfAChain}和{require}中的其他成就，可快速前往{currentZone}类别中成就的默认定位
...
L["Set Keybind Desc"] = "将所选标签的快捷键设置为“Y”键，这将解除“Y”键的其他按键绑定，可前往{keyBindings}中更改为其他按键"
