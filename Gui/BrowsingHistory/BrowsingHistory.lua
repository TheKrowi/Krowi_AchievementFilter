...
function KrowiAF_AchievementFrameBrowsingHistoryPrevNextAchievementButtonMixin:Click()
    ...
    if KrowiAF_Tooltip:GetOwner() == self then
        self:OnEnter();
    end
end
...
local function SetTooltipHistory()
    ...
    GameTooltip_AddBlankLineToTooltip(KrowiAF_Tooltip);
    if #records < 1 then
        local color = addon.Util.Colors.LightGreyRGB;
        KrowiAF_Tooltip:AddLine(addon.L["Select achievements to see your browsing history"], color.R, color.G, color.B);
    ...
    for i = firstIndex, lastIndex do
        ...
        KrowiAF_Tooltip:AddLine(path .. " > " .. addon.GetAchievmentName(records[i].AchievementId), color.R, color.G, color.B);
    end
...
function KrowiAF_AchievementFrameBrowsingHistoryPrevAchievementButtonMixin:OnEnter()
    KrowiAF_Tooltip:SetOwner(self, "ANCHOR_BOTTOMLEFT", 0, self:GetHeight());
    KrowiAF_Tooltip:SetText(addon.L["Go back one achievement"]);
    SetTooltipHistory();
end
...
function KrowiAF_AchievementFrameBrowsingHistoryNextAchievementButtonMixin:OnEnter()
    ...
    KrowiAF_Tooltip:SetText(addon.L["Go forward one achievement"]);
    SetTooltipHistory();
end
