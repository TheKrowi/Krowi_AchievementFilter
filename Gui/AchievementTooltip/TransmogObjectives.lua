...
local function AchievementHasChanged(prevAchievementId)
    local owner = KrowiAF_Tooltip:GetOwner();
...
local function GetTooltipText()
    ...
    for i = 1, KrowiAF_Tooltip:NumLines() do
        r, g, b = _G["KrowiAF_TooltipTextLeft" .. i]:GetTextColor();
        tinsert(linesLeft, {
            Text = _G["KrowiAF_TooltipTextLeft" .. i]:GetText() or "",
            R = r,
            G = g,
            B = b
        });
        r, g, b = _G["KrowiAF_TooltipTextRight" .. i]:GetTextColor();
        tinsert(linesRight, {
            Text = _G["KrowiAF_TooltipTextRight" .. i]:GetText() or "",
            R = r,
            G = g,
            B = b
        });
...
local function CopyPrevTooltipLines(originalOwner)
    ...
    KrowiAF_Tooltip:SetOwner(originalOwner, "ANCHOR_BOTTOMRIGHT", 0, originalOwner:GetHeight());
    local lineLeft, lineRight;
    for i = 1, #linesLeft - 1, 1 do
        lineLeft, lineRight = linesLeft[i], linesRight[i];
        if lineRight == "" then
            KrowiAF_Tooltip:AddLine(lineLeft.Text, lineLeft.R, lineLeft.G, lineLeft.B);
        else
            KrowiAF_Tooltip:AddDoubleLine(lineLeft.Text, lineRight.Text, lineLeft.R, lineLeft.G, lineLeft.B, lineRight.R, lineRight.G, lineRight.B);
        end
    end
end
...
local function AddCriteriaLine(numCollectedPerSet, invTypes)
    ...
    for _, invType in next, invTypes do
        ...
        KrowiAF_Tooltip:AddLine(text);
    end
end

local function AddCriteria()
    local originalOwner = KrowiAF_Tooltip:GetOwner();
    KrowiAF_Tooltip:AddLine(addon.L["Collecting data"]);
...
function section:Add()
    ...
    KrowiAF_Tooltip:AddLine(string.format(addon.Util.Colors.Red, addon.L["This achievement can't be earned by this character."]));
    KrowiAF_Tooltip:AddLine(string.format(addon.Util.Colors.Red, addon.L["This character's class has no transmog set that meet the requirements."]));
end