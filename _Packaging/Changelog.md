...
## 99.10 - 2026-08-21
### Fixed
- `attempt to compare field 'calendarType' (a secret string value, while execution tainted by 'Krowi_AchievementFilter')` on login or `/reload` inside a dungeon or raid (dev note: `C_Calendar.GetDayEvent` is flagged `SecretInChatMessagingLockdown`, so every field of the returned event turns secret while the player is on a communication restricted map or under encounter, Mythic+ or PvP match restrictions. Building the calendar events cache compared `calendarType`, and would then have done arithmetic on `startTime`/`endTime` and stored a secret `title`. The build now probes `canaccessvalue` on the first field it reads, aborts the whole 13 month sweep, restores the calendar's month cursor, and re-arms itself on the next `PLAYER_ENTERING_WORLD` so the cache builds normally once the player leaves the instance)
...
### Changed
- Let Her Solo Me (42799) moved from Voidrazor Sanctuary to main delves category
- Achievement availability filtering for temporary/seasonal content:
  - I TAKE Candle! (40446) - Before Version 11.1.0
  - Just Keep Swimming (40452) - Before Version 11.2.0
  - I've Got a Flying Machine! (40459) - PvE Season 13
  - Flying in Style (40789) - PvE Season 13
  - I've Got a Flying Machine? (41532) - PvE Season 14
  - From Trash to Treasure (41714) - PvE Season 14
  - Rise of the Red Dawn (41779) - Event 1425, not recurring
  - Titan Console Overcharged (42212) - Before PvE Season 15
  - Overcharged Delver (42241) - Before PvE Season 15
  - This Machine Flies?! Don't Care I Got It! (42677) - PvE Season 15
  - So That's Where My Manaflux Capacitor Was! (42678) - PvE Season 15
  - Let Her Solo Me (42799) - PvE Season 15
  - Journey's End Season 3 (42801) - PvE Season 15
  - With Flying Colors variants (60934, 60933) - Before Version 11.2.5
  - Master of the Turbulent Timeways IV (61394) - Event 1425 (added missing flag)

### Fixed
- Master of the Turbulent Timeways achievements now flagged correctly as obtainable
- Nested achievements in zone data should be working as intended now
- Season-specific delve achievements now properly organized into zone data 
