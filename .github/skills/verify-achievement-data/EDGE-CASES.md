...
- **`"X Reward: Required to unlock Y for purchase"` and `"X Reward: Unlock Y for purchase"` still map to the corresponding reward type** — some achievements unlock a reward for purchase from a vendor rather than granting it directly. The DB encodes these as e.g. `"Pet Reward: Required to unlock Silento for purchase"` or `"Pet Reward: Unlock Frosticus Maximus for purchase"`. The reward method is still `:Pet()` (not `:Other()`). Likewise `"Transmog Reward: Required to unlock X for purchase"` maps to `:Transmog()`. The script should treat `"Required to unlock"` and `"Unlock"` sub-phrases as valid tangible-reward signals when prefixed by a known reward type. Observed on Ach(62880, 62882, 63349) `:Pet()` and Ach(62881) `:Transmog()` in `Retail/12_Midnight/AchievementData.lua`.
...
- **Fixture pitfall: `:Mount()` (or `:Title()`/`:HousingDecor()`) with an empty `Reward_lang` is always suppressed as a known DB-gap false positive** — when adding a `known_bad.lua` case for "method present but DB shows no reward", do not pick an ID whose real `Reward_lang` is empty and use one of the three carve-out methods (`:Mount()`, `:Title()`, `:HousingDecor()`), or the FP-suppression branch will swallow it and the fixture will never fail. Use a reward method without a carve-out instead (e.g. `:Toy()`, `:Pet()`, `:Item()`, `:Transmog()`, `:Tabard()`) so the case reaches the genuine `[FAIL]` branch. Discovered on Ach(940) "The Green Hills of Stranglethorn" (real `Reward_lang=""` in WotLK Classic build 5.5.4.68806) — fixed by switching the fixture's tag from `:Mount()` to `:Toy()`. Observed in `fixtures/known_bad.lua`.
